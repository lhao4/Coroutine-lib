...
| 模块 | 文件位置 | 模块职责 |
|---|---|---|
| coroutine/fiber | `include/mycoroutine/fiber.h` `src/fiber.cpp` | 协程对象、汇编上下文切换、共享栈、嵌套调用 |
| context | `include/mycoroutine/context.h` `src/context_switch_x86_64.S` `src/context_switch_aarch64.S` | 汇编上下文切换函数与 FiberContext 结构定义 |
| scheduler | `include/mycoroutine/scheduler.h` `src/scheduler.cpp` | deque 任务队列、线程调度、策略选取、协程池接入 |
| coroutine_pool | `include/mycoroutine/coroutine_pool.h` `src/coroutine_pool.cpp` | 复用终止 Fiber，控制缓存容量 |
| sync | `include/mycoroutine/sync.h` | 协程同步原语：CoMutex、WaitGroup、Channel\<T\> |
...
### 2.1 coroutine/fiber
- 文件位置：`include/mycoroutine/fiber.h`，`src/fiber.cpp`，`include/mycoroutine/context.h`，`src/context_switch_x86_64.S`，`src/context_switch_aarch64.S`
- 模块职责：
  - 管理协程状态机：`READY/RUNNING/TERM`
  - 通过汇编函数 `mycoroutine_context_swap` 实现 `resume/yield` 切换（仅保存/恢复 callee-saved 寄存器，无系统调用）
  - 集成 ASan/TSan fiber switch hook，保证 Sanitizer 正确追踪协程栈
  - 提供共享栈槽位占用切换与栈快照
...
### 2.4 sync
- 文件位置：`include/mycoroutine/sync.h`（header-only）
- 模块职责：
  - 提供协程级同步原语，等待时挂起协程而非阻塞工作线程
  - `CoMutex`：协程互斥锁，竞争时协程 yield，释放时唤醒等待队列首个协程
  - `WaitGroup`：类似 Go WaitGroup，计数归零时批量唤醒所有 `wait()` 协程
  - `Channel<T>`：有界/无界通道，`capacity=0` 为同步通道，支持 `close()` 语义
- 核心接口：
  - `CoMutex::lock()` / `tryLock()` / `unlock()`
  - `WaitGroup::add(delta)` / `done()` / `wait()`
  - `Channel<T>::send(value)` / `recv(out)` / `close()`
- 内部机制：
  - 统一等待节点 `FiberWaiter`：保存当前 Fiber、SchedulerRef 弱引用、线程亲和 ID
  - 唤醒时通过 `SchedulerRef::schedule()` 将协程重新入队，保证调度器生命周期安全
- 调用关系：
  - 依赖 `Fiber::GetThis()` 获取当前协程
  - 依赖 `Scheduler::getSchedulerRef()` 安全回调调度器
