...
协程创建本质步骤：
- 分配栈内存（`malloc`）
- 构造 `FiberContext`：对齐栈顶、写入入口地址（`FiberEntryPoint`）、初始化 callee-saved 寄存器
- 切换时通过汇编函数 `mycoroutine_context_swap` 保存/恢复寄存器与栈指针
...
flowchart TD
    ...
    D --> E[malloc stack]
    E --> F[align stack top]
    F --> G[write FiberEntryPoint address]
    G --> H[init FiberContext registers]
...
切换基于汇编函数 `mycoroutine_context_swap(from_ctx, to_ctx)`，仅保存/恢复 callee-saved 寄存器和栈指针，无系统调用开销。`m_runInScheduler` 决定 yield 时切回调度协程还是线程主协程。

切换时还会调用 ASan/TSan 的 fiber switch hook（`__sanitizer_start/finish_switch_fiber`、`__tsan_switch_to_fiber`），保证 Sanitizer 正确追踪协程栈。
...
1. 完成 ASan fiber switch（`__sanitizer_finish_switch_fiber`）
2. 执行回调（带异常保护）
3. `m_cb=nullptr`
4. `m_state=TERM`
5. `yield()` 返回调度侧
